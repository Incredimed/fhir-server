...
namespace Microsoft.Health.Fhir.MongoDb.UnitTests.Features.Storage.Versioning
{
    public class CollectionUpgradeManagerTests
    {
        private readonly MongoDataStoreConfiguration _mongoDataStoreConfiguration = new MongoDataStoreConfiguration
        {
        ...
        {
            var factory = Substitute.For<IMongoDbDistributedLockFactory>();
            var mongoDbDistributedLock = Substitute.For<IMongoDbDistributedLock>();
            var collectionVersionWrappers = Substitute.For<IQueryable<CollectionVersion>, IDocumentQuery<CollectionVersion>>();

            factory.Create(Arg.Any<IDocumentClient>(), Arg.Any<Uri>(), Arg.Any<string>()).Returns(mongoDbDistributedLock);
            mongoDbDistributedLock.TryAcquireLock().Returns(true);
            ...
            var updaters = new ICollectionUpdater[] { new CollectionSettingsUpdater(NullLogger<CollectionSettingsUpdater>.Instance, _mongoDataStoreConfiguration), };
            _manager = new CollectionUpgradeManager(updaters, _mongoDataStoreConfiguration, factory, NullLogger<CollectionUpgradeManager>.Instance);
        }
        ...
        public async Task GivenACollection_WhenSettingUpCollection_ThenTheCollectionVersionWrapperIsSaved()
        {
            ...
            await _client.Received(1).UpsertDocumentAsync(Arg.Is(_mongoDataStoreConfiguration.RelativeCollectionUri), Arg.Is<CollectionVersion>(x => x.Version == CollectionUpgradeManager.CollectionSettingsVersion));
        }
