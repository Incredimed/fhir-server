...
namespace Microsoft.Health.Fhir.MongoDb.Features.Consistency
{
    ...
    internal sealed partial class DocumentClientWithConsistencyLevelFromContext
    {
        ...
        private (ConsistencyLevel? consistencyLevel, string sessionToken) GetConsistencyHeaders()
        {
            ...
            if (fhirRequestContext.RequestHeaders.TryGetValue(MongoDbConsistencyHeaders.ConsistencyLevel, out var values))
            {
            ...
            fhirRequestContext.RequestHeaders.TryGetValue(MongoDbConsistencyHeaders.SessionToken, out values);
        ...
        private void AddSessionTokenToResponseHeaders(string sessionToken)
        {
            if (!string.IsNullOrEmpty(sessionToken))
            {
                ...
                if (fhirRequestContext != null)
                {
                    fhirRequestContext.ResponseHeaders[MongoDbConsistencyHeaders.SessionToken] = sessionToken;
                }
            }
        }
