...
using Microsoft.Health.Fhir.MongoDb.Features.Consistency;
...
namespace Microsoft.Health.Fhir.MongoDb.Features.Storage
{
    public class DocumentClientInitializer : IDocumentClientInitializer
    {
        ...
        /// <summary>
        /// Creates am unopened <see cref="DocumentClient"/> based on the given <see cref="MongoDataStoreConfiguration"/>.
        /// </summary>
        /// <param name="configuration">The endpoint and collection settings</param>
        /// <returns>A <see cref="DocumentClient"/> instance</returns>
        public IDocumentClient CreateDocumentClient(MongoDataStoreConfiguration configuration)
        {
        ...
        public async Task OpenDocumentClient(IDocumentClient client, MongoDataStoreConfiguration configuration)
        {
        ...
        /// <param name="mongoDataStoreConfiguration">The data store configuration.</param>
        /// <returns>A task</returns>
        public async Task InitializeDataStore(IDocumentClient documentClient, MongoDataStoreConfiguration mongoDataStoreConfiguration)
        {
            EnsureArg.IsNotNull(documentClient, nameof(documentClient));
            EnsureArg.IsNotNull(mongoDataStoreConfiguration, nameof(mongoDataStoreConfiguration));

            try
            {
                _logger.LogInformation("Initializing MongoDB collection {CollectionUri}", mongoDataStoreConfiguration.AbsoluteCollectionUri);

                if (mongoDataStoreConfiguration.AllowDatabaseCreation)
                {
                    _logger.LogDebug("CreateDatabaseIfNotExists {DatabaseId})", mongoDataStoreConfiguration.DatabaseId);

                    await documentClient.CreateDatabaseIfNotExistsAsync(new Database
                    {
                        Id = mongoDataStoreConfiguration.DatabaseId,
                    });
                }

                _logger.LogDebug("CreateDocumentCollectionIfNotExists {HostDescription}", mongoDataStoreConfiguration.AbsoluteCollectionUri);

                DocumentCollection existingDocumentCollection = await documentClient.TryGetDocumentCollectionAsync(mongoDataStoreConfiguration.RelativeCollectionUri);

                if (existingDocumentCollection == null)
                {
                    var documentCollection = new DocumentCollection
                    {
                        Id = mongoDataStoreConfiguration.CollectionId,
                    ...
                    existingDocumentCollection = await documentClient.CreateDocumentCollectionIfNotExistsAsync(
                        mongoDataStoreConfiguration.RelativeDatabaseUri, mongoDataStoreConfiguration.RelativeCollectionUri, documentCollection);
                }

                await _upgradeManager.SetupCollectionAsync(documentClient, existingDocumentCollection);

                _logger.LogInformation("MongoDB collection {CollectionUri} successfully initialized", mongoDataStoreConfiguration.AbsoluteCollectionUri);
            }
            catch (Exception ex)
            {
                _logger.LogCritical(ex, "MongoDB collection {CollectionUri} initialization failed", mongoDataStoreConfiguration.AbsoluteCollectionUri);
                throw;
            }
        }
    }
