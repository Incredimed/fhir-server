...
namespace Microsoft.Health.Fhir.MongoDb.Features.Storage
{
    ...
    public class DocumentClientProvider : IStartable, IRequireInitializationOnFirstRequest, IDisposable
    {
            ...
            ILogger<DocumentClientProvider> logger)
        {
            ...
            _documentClient = documentClientInitializer.CreateDocumentClient(mongoDataStoreConfiguration);

            _initializationOperation = new RetryableInitializationOperation(
                () => documentClientInitializer.InitializeDataStore(_documentClient, mongoDataStoreConfiguration));
        }
