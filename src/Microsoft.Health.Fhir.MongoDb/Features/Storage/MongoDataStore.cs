...
using Microsoft.Health.Fhir.MongoDb.Features.Storage.StoredProcedures.HardDelete;
using Microsoft.Health.Fhir.MongoDb.Features.Storage.StoredProcedures.Upsert;
using Task = System.Threading.Tasks.Task;

namespace Microsoft.Health.Fhir.MongoDb.Features.Storage
{
    public sealed class MongoDataStore : IDataStore, IContinuationTokenCache, IProvideCapability
    {
        private readonly IScoped<IDocumentClient> _documentClient;
        private readonly IMongoDocumentQueryFactory _mongoDocumentQueryFactory;
        ...
        /// <param name="mongoDocumentQueryFactory">The factory used to create the document query.</param>
        ...
        public MongoDataStore(
            ...
            MongoDataStoreConfiguration mongoDataStoreConfiguration,
            IMongoDocumentQueryFactory mongoDocumentQueryFactory,
            RetryExceptionPolicyFactory retryExceptionPolicyFactory,
            ILogger<MongoDataStore> logger)
        {
            EnsureArg.IsNotNull(documentClient, nameof(documentClient));
            EnsureArg.IsNotNull(mongoDataStoreConfiguration, nameof(mongoDataStoreConfiguration));
            EnsureArg.IsNotNull(mongoDocumentQueryFactory, nameof(mongoDocumentQueryFactory));
            EnsureArg.IsNotNull(retryExceptionPolicyFactory, nameof(retryExceptionPolicyFactory));
            EnsureArg.IsNotNull(logger, nameof(logger));

            _mongoDocumentQueryFactory = mongoDocumentQueryFactory;
            _retryExceptionPolicyFactory = retryExceptionPolicyFactory;
            _logger = logger;
            _documentClient = documentClient;
            _collectionUri = mongoDataStoreConfiguration.RelativeCollectionUri;
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var mongoWrapper = new MongoResourceWrapper(resource);
        ...
        public async Task<ResourceWrapper> GetAsync(ResourceKey key, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var executor = CreateDocumentQuery<MongoResourceWrapper>(
                sqlQuerySpec,
                new FeedOptions { PartitionKey = new PartitionKey(key.ToPartitionKey()) });

            var result = await executor.ExecuteNextAsync<MongoResourceWrapper>(cancellationToken);
            ...
            FeedOptions feedOptions = null)
        {
            ...
            MongoQueryContext context = new MongoQueryContext(_collectionUri, sqlQuerySpec, feedOptions);

            return _mongoDocumentQueryFactory.Create<T>(_documentClient.Value, context);
        }
        ...
        public async Task<string> GetContinuationTokenAsync(string id, CancellationToken cancellationToken = default)
        {
            ...
            IDocumentQuery<ContinuationToken> mongoDocumentQuery = CreateDocumentQuery<ContinuationToken>(ctQuery, ctOptions);

            using (mongoDocumentQuery)
            {
                var response = await mongoDocumentQuery.ExecuteNextAsync(cancellationToken);
                ContinuationToken result = response.SingleOrDefault();

                if (result == null)
                {
                    _logger.LogError("Continuation token does not exist in MongoDB.");
        ...
        public async Task<string> SaveContinuationTokenAsync(string continuationToken, CancellationToken cancellationToken = default)
        {
            ...
            using (_logger.BeginTimedScope($"{nameof(MongoDataStore)}.{nameof(SaveContinuationTokenAsync)}"))
            {
